...
@njit
def contorno(y, y0, v, dt, dx, primeiro_passo=False):
    # Condições de contorno (extremidades fixas)
    y[:, 0] = 0  # Extremidade esquerda fixa
    y[:, -1] = 0  # Extremidade direita fixa

    # Aplicar a condição inicial para y(x, t = Δt)
    if primeiro_passo:
        y[0, :] = y0  # Condição inicial y(x, 0)
        y[1, :] = np.interp(np.arange(len(y0)) * dx - v * dt, np.arange(len(y0)) * dx, y0)
...
def resolver_onda(y0, Nx, Nt, dt, dx, v):
    ...
    # Aplicar condições iniciais e de contorno no primeiro passo
    y = contorno(y, y0, v, dt, dx, primeiro_passo=True)

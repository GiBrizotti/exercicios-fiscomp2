...
def parametros_polares(rmin, rmax, Nr, Ntheta):
    ...
    return r, theta, R, Theta, dr, dtheta, dr2, dtheta2

def contorno(V, m, theta):
    V[:, 0] = 0
    V[:, -1] = np.cos(m*theta)
    return V
...
def simular(R, Theta, V):
    ...
    ax.plot_surface(X, Y, V, cmap='gnuplot')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('f(x, y)')
...
def simular2d(R, Theta, V):
    ...
    plt.contourf(X, Y, V, cmap='gnuplot')
    plt.colorbar()
    plt.xlabel('x')
    plt.ylabel('y')
...
def convergencia(rmin, rmax, Nr, Ntheta, delta, m):
    r, theta, R, Theta, dr, dtheta, dr2, dtheta2 = parametros_polares(rmin, rmax, Nr, Ntheta)
    V = np.zeros((Nr, Ntheta))
    V= contorno(V, m, theta)
    ...
    for iter in range(kmax):
        ...
        V_iter = contorno(V_iter, m, theta)
...
def main():
    ...
    m = [1,2,7]
    for i in m:
        convergencia(rmin, rmax, Nr, Ntheta, delta, i)
